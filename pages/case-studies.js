...
const ACCENT = { creators: '#8b5cf6', agencies: '#3b82f6', brands: '#f59e0b', saas: '#10b981', schools: '#0ea5e9' }
...
export default function CaseStudies() {
  ...
  useEffect(() => {
    ...
    if (typeof t === 'string' && caseStudies.some((c) => c.id === t)) setActive(t)
  }, [router.query.tab])

  const selectTab = (id) => {
    setActive(id)
    router.push({ pathname: '/case-studies', query: { tab: id } }, undefined, { shallow: true, scroll: false })
  }

  const study = caseStudies.find((c) => c.id === active) || caseStudies[0]
  const accent = ACCENT[study.id]
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-page pt-[68px]">
        {/* Compact hero */}
        <section className="max-w-4xl mx-auto px-4 sm:px-6 text-center pt-14 lg:pt-16 pb-8">
              ...
              {caseStudies.map((t) => {
                ...
                return (
                  <button key={t.id} onClick={() => selectTab(t.id)}
                    ...
                    style={on ? { background: `color-mix(in srgb, ${c} 14%, transparent)` } : undefined}>
                    <TabIcon style={{ color: on ? c : 'currentColor', fontSize: 16 }} />
                    {t.label}
                  </button>
                )
              })}
          ...
          >
            ...
            <section className="max-w-7xl mx-auto px-4 sm:px-6 pb-14 relative z-10">
              <div className="max-w-2xl mb-8">
                ...
                <h3 className="font-display text-2xl md:text-3xl font-extrabold text-body tracking-[-0.02em] leading-[1.1] mt-2">{study.pitch}</h3>
              </div>
                ...
                {study.benefits.map((b) => {
                  ...
                  return (
                    <div key={b.title} className="rounded-2xl border border-line bg-surface p-5 transition-colors hover:border-line-strong">
                      ...
                      <h4 className="font-display font-bold text-body text-[15px] mb-1.5">{b.title}</h4>
            ...
            {/* Before / After + Workflow for the active audience */}
